...
mekukey = '2M94FZ0CPSe0lum' //3000 LIMIT
...
namaowner = 'Aprii'
fake = '_*BY BOT INDO, KETIK .allmenu JIKA TIDAK ADA BUTTON*_'
apri = '628811929177'
...
buff = await getBuffer (anu.result.nowm)
client.sendMessage(from, buff, video, {mimetype: 'video/mp4', quoted: mek, caption: 'SUBS BOT INDO SU'})
break
case 'tiktokwm':
if (!isUser) return reply(mess.daftar)
if (isBanned) return reply (mess.banned)
reply(mess.wait)
anu = await fetchJson (`https://api-aprilia-xyz.herokuapp.com/api/tiktok?url=${args.join(' ')}`)
buff = await getBuffer (anu.result.original)
client.sendMessage(from, buff, video, {mimetype: 'video/mp4', quoted: mek, caption: 'SUBS BOT INDO SU'})
break
/*
case 'tiktoknowm':
...
if (!isUser) return reply(mess.daftar)
			if (isBanned) return reply(mess.banned)
					...
					reply(mess.wait)
					buffer = await getBuffer(`https://api.mekuhy.ga/api/textpro/glitch2?text1=${gli}&text2=${tch}&apikey=${mekukey}`)
...
reply(mess.wait)
anu = await fetchJson(`https://api.mekuhy.ga/api/socialmedia/igdownload?url=${args.join(' ')}&apikey=2M94FZ0CPSe0lum`)
...
if (!isUser) return reply(mess.daftar)
      ...
      anu = await fetchJson(`https://api.mekuhy.ga/api/socialmedia/ytplaymp3?query=${args.join(' ')}&apikey=2M94FZ0CPSe0lum`)
      reply(mess.wait)
...
if (!isUser) return reply(mess.daftar)
      if (isBanned) return reply (mess.banned)
      if (args.length < 1) return reply("```JUDUL MANA TOT```")
      anu = await fetchJson(`https://api.mekuhy.ga/api/socialmedia/ytplaymp4?query=${args.join(' ')}&apikey=2M94FZ0CPSe0lum`)
      reply(mess.wait)
...
reply (mess.wait)
anu = await fetchJson (`https://api.mekuhy.ga/api/socialmedia/ytmp3?url=${args.join(' ')}&apikey=2M94FZ0CPSe0lum`)
...
if (!isUrl(args[0]) && !args[0].includes('youtu.be')) return reply('```LINK NYA JELEK NIH```')
reply (mess.wait)
anu = await fetchJson (`https://api.mekuhy.ga/api/socialmedia/ytmp4?url=${args.join(' ')}&apikey=2M94FZ0CPSe0lum`)
captionnya = `TITLE BERHASIL DImekATKAN\n\nJudul : ${anu.result.title}\nSize : ${anu.result.size}\nKualitas Video : ${anu.result.quality}\nViews : ${anu.result.views}\nChannel : ${anu.result.channel}\nDeskripsi : ${anu.result.desc}\n\n*MOHON TUNGGU BEBERAPA MENIT BOT SEDANG MENGIRIM VIDEO*`
buffer = await getBuffer (anu.result.thumb)
client.sendMessage(from, buffer, image, {quoted: mek, caption: captionnya})
miku = await getBuffer (anu.result.url)
...
case 'join':  case 'joingc':
   ...
   break
        ...
        case 'quotes':
        if (isBanned) return reply (mess.banned)
        anu = await fetchJson(`https://melcanz.com/api/quotes?apikey=daff`)
        reply (anu.quotes)
        break
                case 'cerpen':
                if(isBanned) return reply (mess.banned)
               ...
               anu = await fetchJson(`https://melcanz.com/api/artinama?nama=${args.join(' ')}&apikey=daff`)
               reply(anu.result)
               break
               case 'artimimpi':
               if (isBanned) return reply (mess.banned)
               if (args.length == 0) return reply(`Example: ${prefix + command} apri`)
               anu = await fetchJson(`https://melcanz.com/api/artimimpi?mimpi=${args.join(' ')}&apikey=daff`)
               reply(anu.result)
               break
               case 'kisahnabi':
               if (isBanned) return reply (mess.banned)
...
case "closegc": 
		...
		case 'cantikcek':
          ...
          if (isBanned) return reply (mess.banned)
                if (args.length == 0) return reply(`Example: ${prefix + command} BotIndo`)
                    txt = args.join(" ")
                    reply(mess.wait)
                    buffer = await getBuffer(`https://melcanz.com/rem?text=${txt}&apikey=daff`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: mess.success})
                    break
           case 'guralogo':
           if (isBanned) return reply (mess.banned)
           ...
           if (isBanned) return reply (mess.banned)
                if (args.length == 0) return reply(`Example: ${prefix + command} Bot | Indo`)
                    txt = args.join(" ")
                    reply(mess.wait)
                    buffer = await getBuffer(`https://api.mekuhy.ga/api/maker/nulis?text=${txt}&apikey=2M94FZ0CPSe0lum`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: mess.success})
                    break
            case 'glitch':
            if (isBanned) return reply(mess.banned)
           ...
           if (isBanned) return reply(mess.banned)
            ...
            if (isBanned) return reply(mess.banned)
             ...
             if (isBanned) return reply(mess.banned)
					...
					if (args.length < 1) return reply(`Example: ${prefix + command}Bot&Indo`)
					reply(mess.wait)
					buffer = await getBuffer(`https://melcanz.com/girlneko?text=${gli}&text2=${tch}&apikey=daff`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: mess.success})
					break
             case 'sadlogo':
             if (isBanned) return reply(mess.banned)
					...
					if (args.length < 1) return reply(`Example: ${prefix + command}Bot&Indo`)
					reply(mess.wait)
					buffer = await getBuffer(`https://melcanz.com/sadboy?text=${gli}&text2=${tch}&apikey=daff`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: mess.success})
					break
            case 'lighttext':
            if (isBanned) return reply (mess.banned)
   ...
   if (isBanned) return reply (mess.banned)
                if (args.length == 0) return reply(`Example: ${prefix + command} BotIndo`)
                    txt = args.join(" ")
                    reply(mess.wait)
                    buffer = await getBuffer(`https://melcanz.com/kaneki?text=${txt}&apikey=daff`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: mess.success})
                    break
           case 'lavatext':
           if (isBanned) return reply (mess.banned)
                if (args.length == 0) return reply(`Example: ${prefix + command} BotIndo`)
                    txt = args.join(" ")
                    reply(mess.wait)
                    buffer = await getBuffer(`https://api.mekuhy.ga/api/textpro/lava?text=${txt}&apikey=2M94FZ0CPSe0lum`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: mess.success})
                    break
           case 'snowtext':
           if (isBanned) return reply (mess.banned)
                if (args.length == 0) return reply(`Example: ${prefix + command} BotIndo`)
                    txt = args.join(" ")
                    reply(mess.wait)
                    buffer = await getBuffer(`https://api.mekuhy.ga/api/textpro/snow?text=${txt}&apikey=2M94FZ0CPSe0lum`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: mess.success})
                    break
           case 'lihttext':
             if (isBanned) return reply (mess.banned)
            ...
            if (isBanned) return reply (mess.banned)
              ...
              if (isBanned) return reply (mess.banned)
                if (args.length == 0) return reply(`Example: ${prefix + command} BotIndo`)
                    txt = args.join(" ")
                    reply(mess.wait)
                    buffer = await getBuffer(`https://melcanz.com/lolimaker?text=${txt}&apikey=daff`)
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: mess.success})
                    break
              case 'logoepep':
              if (isBanned) return reply (mess.banned)
...
Wm Bot Indo`
            ...
            case 'waifu':
             ...
             case 'nekoh':
               ...
               reply(mess.wait)
                ...
                case 'yuri':
                if (isBanned) return reply (mess.banned)
                buffer = await getBuffer (`https://api.mekuhy.ga/api/nsfw/nsfwyuri?apikey=${mekukey}`)
            ...
            await client.sendMessage(from, ButonMesages, MessageType.buttonsMessage, {quoted : mek})
                ...
                if (isBanned) return reply (mess.banned)
               reply(mess.wait)
                buffer = await getBuffer(`https://api.mekuhy.ga/api/nsfw/nsfwfutanari?apikey=${mekukey}`)
   ...
   sendButImage(from, pll, `Klik Next Untuk Melanjutkan`, kon, [                      
          {
            ...
            case 'sagiri':
				...
				if (isBanned) return reply(mess.banned)
					...
					if (args.length < 1) return reply(`Example: ${prefix + command} Bot&Indo`)
					reply(mess.wait)
					buffer = await getBuffer(`https://api.mekuhy.ga/api/textpro/pornhub?text1=${gli}&text2=${tch}&apikey=2M94FZ0CPSe0lum`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: mess.success})
					break
                case 'ytsearch':
...
tod = await getBuffer(`https://telegra.ph/file/93fd4cb214d27997aec65.jpg`)
                 ...
                 break
                case 'sc':
                reply(`*YANG PENTING BUKAN SC YT OKE ATAU GITHUB*`)
                break
   case 'bokep':
